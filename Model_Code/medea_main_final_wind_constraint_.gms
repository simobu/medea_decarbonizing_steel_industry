...
* ------------------------------------------------------------------------------
* command-line option --PROJECT: --PROJECT="contrain_invest" gdx=renewables_only
...
$if NOT exist MEDEA_%scenario%_data.gdx  $gdxin medea_main_re_only
...
$load    PRICE PRICE_CO2 PRICE_TRADE PROFILE SIGMA VALUE_NSE
$gdxin

SWITCH_INVEST = 10000; #allow basically infinite investment
...
CAPITALCOST(z,t) = OVERNIGHTCOST(t)*1000 * ANNUITY_FACTOR(z,t);#include correction from kW to MW
CAPITALCOST_E(z,s) = OVERNIGHTCOST_E(s)*1000 * ANNUITY_FACTOR(z,s);#include correction from kW to MW
CAPITALCOST_P(z,s) = OVERNIGHTCOST_P(s)*1000 * ANNUITY_FACTOR(z,s);#include correction from kW to MW
CAPITALCOST_X(z,f) = OVERNIGHTCOST_X(f)*1000 * ANNUITY_FACTOR_X(z,f);#include correction from kW to MW
...
Positive Variables
        ...
        cost_air_pol(z,i)           cost of air pollution [t EUR]
        cost_inputs(z,i,t)          cost of inputs [t EUR]
        cost_emissions(z,i)         cost of CO2 emissions [t EUR]
        cost_om(z,t)                cost of operation and maintenance [t EUR]
        cost_invest(z,t)            cost of investment [t EUR]
        cost_grid(z,f)              cost of grid expansion [t EUR]
        cost_nse(z,f)               cost of non-served demand [t EUR]
        cost_trade(z,f)             cost of trade in energy carriers [t EUR]
        cost_stor_volume(z,s)       cost of adding storage volume [t EUR]
        curtail(z,h,f)              discarded energy output
        decommission(z,t)           capacity decommissioned
        emission_co2(z,h,i)         quantity of CO2 emitted
        fuel_trade(z,h,f)           fuel shipment [unit?]
        gen(z,h,f,t)                energy generation
        gen_biomass(z,f,t)          annual electricity generation from biomass
        invest(z,t)                 capacity investment [GW]
        nse(z,h,f)                  non-served energy
        storage_cap_invest(z,s)     added storage volume [GWh]
        storage_cap_neg_invest(z,s) added storage volume [GWh]
        storage_loss(z,h,f,s)       thermal storage loss
        use(z,h,e,t)                energy use (endogenous)
        w(z,h,c,l,i)                co-generation weight
        import_max(z,f)             hourly max h2 import [GW]
        sum_gen(z,f,t)              annual generation
        sum_use_el(z,e,t)           annual el use
        sum_use_h2(z,e,t)           annual h2 use
;
* ==============================================================================
...
acc_storage_volume, balance, intermittent_generation, energy_conversion, capacity_constraint,
...
bal_co2, acn_airpollute,limit_curtail,ancillary_services, store_in_out_limit,
storage_loss_thermal,storage_loss_battery,
 storage_limit_neg, storage_balance_h1, biomass_constraint, h2_import_constraint, h2_import_hourly,
summary_gen, summary_use_el, summary_use_h2
;
* ==============================================================================
...
* ------------------------------------------------------------------------------
* CONSTRAIN VARIABLES
#make sure technologies do not generate other energy carriers
gen.UP(z,h,f,t)$(not MAP_OUTPUTS(f,t)) = 0;
gen_biomass.UP(z,f,t) = 0;
gen_biomass.UP('AT','el','bio') = 6666;#6,66 TWh = 24 PJ
gen_biomass.UP('DE','el','bio') = 56000; #56 TWh
# make sure technologies use only those energy carriers that they can use
use.UP(z,h,i,t)$(not MAP_INPUTS(i,t)) = 0;
# restrict co-generation for non co generators
w.UP(z,h,c,l,i)$(NOT MAP_INPUTS(i,c)) = 0;
w.UP(z,h,c,l,i)$(NOT FEASIBLE_INPUT(l,i,c)) = 0;
#only storages can actually stor sth
storage_content.UP(z,h,f,s)$(NOT MAP_OUTPUTS(f,s)) = 0;
storage_content.UP(z,h,f,s)$(NOT MAP_INPUTS(f,s)) = 0;
# fix storage content for last hour at 0 
storage_content.FX(z,'h8760',f,s) = 0;#fix storage content at end of the year fix initial storage content
#storage loss for all storages is set to 0
storage_loss.UP(z,h,f,s) = 0;
#exception: hw_tank and battery can actually have storage losses
storage_loss.UP(z,h,'el','battery') = 200000;
storage_loss.UP(z,h,'ht','hw_tank') = 200000;
# prohibit energy exchange within the same zone
x.FX(z,zz,h,f)$SAMEAS(z,zz) = 0;
x.FX(zz,z,h,f)$SAMEAS(z,zz) = 0;
# heat energy cannot be exchanged between zones
x.FX(zz,z,h,'ht') = 0;
# irrelevant for this study: only CO2 emitting technologies can produce CO2 emissions
emission_co2.UP(z,h,i)$(NOT CO2_INTENSITY(i)) = 0;
#only renewables can be curtailed
*curtail.UP(z,h,f)$(NOT MAP_OUTPUTS(f,r)) = 0;
#only fuels with a defined price can be imported
fuel_trade.UP(z,h,f)$(NOT PRICE_TRADE(f)) = 0;
#decommission cannot exceed installed capacity
decommission.UP(z,t) = CAPACITY(z,t);
#restrict investment
#invest.UP(z,t) = SWITCH_INVEST; #define investment limits in GW
# restrict exchange capacity between zones (initial capacity only for el defined)
add_x.UP(z,zz,'el') = 0;
add_x.UP(z,zz,'h_2') = 0; #assumes no exchange capacity for h_2
#prohibit additional storage capacity in hydro storages
storage_cap_invest.UP(z,'hyd_psp_season') = 0;
storage_cap_invest.UP(z,'hyd_psp_week') = 0;
storage_cap_invest.UP(z,'hyd_psp_day') = 0;
storage_cap_neg_invest.UP(z,'hyd_psp_season') = 0;
storage_cap_neg_invest.UP(z,'hyd_psp_week') = 0;
storage_cap_neg_invest.UP(z,'hyd_psp_day') = 0;
storage_cap_invest.UP(z,'hyd_res_season') = 0;
storage_cap_invest.UP(z,'hyd_res_week') = 0;
storage_cap_invest.UP(z,'hyd_res_day') = 0;
storage_cap_neg_invest.UP(z,'hyd_res_season') = 0;
storage_cap_neg_invest.UP(z,'hyd_res_week') = 0;
storage_cap_neg_invest.UP(z,'hyd_res_day') = 0;
#disallow transmission to use energy (does not make sense)
use.FX(z,h,i,'transmission')= 0;
#wind constraint
invest.UP(z,'Wind_On') = 0;
invest.UP('DE','Wind_Off') = 15;
invest.UP(z,'ror')=0;
* ------------------------------------------------------------------------------
...
acc_invest(z,t)..
         ...
         ;
acc_storage_volume(z,s)..
         cost_stor_volume(z,s)
         =E=
         CAPITALCOST_E(z,s) * storage_cap_invest(z,s)
         + CAPITALCOST_E(z,s) * storage_cap_neg_invest(z,s)
         ;
acc_grid(z,f)..
         cost_grid(z,f)
         =E=
...
;
*biomass constraint
biomass_constraint(z,'el','bio')..
         gen_biomass(z,'el','bio')
         =E=
         sum(h,gen(z,h,'el','bio'))
         + sum(h,gen(z,h,'el','bio_chp'))
;
...
;
*limit hourly fuel imports
h2_import_constraint(z,h,f)..
         fuel_trade(z,h,f)
         =L=
         import_max(z,f)
;
h2_import_hourly(z,f)..
         import_max(z,f)
         =E=
         sum(h,fuel_trade(z,h,f))/8760
;
* ------------------------------------------------------------------------------
...
* ------------------------------------------------------------------------------
* ENERGY STORAGE
storage_balance(z,h,f,s)$(MAP_INPUTS(f,s)  AND (ord(h) > 1))..
         ...
         - storage_loss(z,h-1,f,s)
;
*define storage balance for 'h1'
storage_balance_h1(z,h,f,s)$(MAP_INPUTS(f,s)  AND (ord(h) = 1))..
         storage_content(z,h,f,s)
         =E=
         INFLOWS(z,h,s)
         + CONVERSION(f,f,s) * use(z,h,f,s)
         - (1/CONVERSION(f,f,s)) * gen(z,h,f,s)
         + storage_content(z,'h8760',f,s)
         - storage_loss(z,'h8760',f,s)
;
*track upper and lower limit of storage capacity expansion
storage_limit(z,h,f,s)$MAP_INPUTS(f,s)..
         storage_content(z,h,f,s)
         =L=
         0.4*CAPACITY_STORAGE(z,f,s) + storage_cap_invest(z,s)
;
storage_limit_neg(z,h,f,s)$MAP_INPUTS(f,s)..
         storage_content(z,h,f,s)
         =G=
         - (0.6*CAPACITY_STORAGE(z,f,s) + storage_cap_neg_invest(z,s))
;
*define hourly losses
storage_loss_thermal(z,h,f,s)$(MAP_INPUTS('ht','hw_tank') AND MAP_OUTPUTS('ht','hw_tank'))..
         storage_loss(z,h,'ht','hw_tank')
         =E=
         storage_cap_neg_invest(z,'hw_tank')*1/12000
         + 0.6*CAPACITY_STORAGE(z,'ht','hw_tank')*1/12000
         + storage_content(z,h,'ht','hw_tank')*1/12000
;
storage_loss_battery(z,h,f,s)$(MAP_INPUTS('el','battery') AND MAP_OUTPUTS('el','battery'))..
         storage_loss(z,h,'el','battery')
         =E=
         storage_cap_neg_invest(z,'battery')*1/24000
         + 0.6*CAPACITY_STORAGE(z,'el','battery')*1/24000
         + storage_content(z,h,'el','battery')*1/24000
;
*limit energy that can go in and out of the storage
...
;
*prevent charging and discharging of the same GW at the same time (rough fix: the same GW can either charge or discharge)
store_in_out_limit(z,h,f,s)$MAP_INPUTS(f,s)..
         use(z,h,f,s) + gen(z,h,f,s)
         =L=
         CAPACITY_STORE_OUT(z,f,s) + invest(z,s) - decommission(z,s)
;
* ------------------------------------------------------------------------------
...
summary_gen(z,f,t)..
         ...
         sum(h,gen(z,h,f,t))
;
summary_use_el(z,'el',t)..
         sum_use_el(z,'el',t)
         =E=
         sum(h,use(z,h,'el',t))
;
summary_use_h2(z,'h_2',t)..
         sum_use_h2(z,'h_2',t)
         =E=
         sum(h,use(z,h,'h_2',t))
;
...
AnnDem(z,f)                      annual demand per zone and energy carrier
*StorageLevel(z,h,f,s)           actual storage level        
;
* ------------------------------------------------------------------------------
...
*AnnG(z,f) = sum((h,t), gen(z, h, f, t));
*AnnGByTec(z,i,f,f) = sum(t, g.L(z, t, i, f, f));
*AnnGByTec(z,f,t) = sum(h, gen(z, h, f, t));
*AnnDem(z,f) = sum(h, DEMAND(z,h,f));
